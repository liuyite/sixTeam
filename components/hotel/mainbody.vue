    ...
    mounted() {
        ...
        window.onLoad = () => {
            ...
            var map = new AMap.Map("container", {
                zoom: 7, //级别
                center: [
                    this.form[0].location.longitude,
                    this.form[0].location.latitude
                ] //中心点坐标
            });
            ...
            for (let i = 0; i < this.form.length; i++) {
                var marker1 = new AMap.Marker({
                    ...
                    position: new AMap.LngLat(
                        this.form[i].location.longitude,
                        this.form[i].location.latitude
                    ),
                    title: `${this.form[i].name}`,
                    content: `<div style="width:20px; height:35px; background:url('https://webapi.amap.com/theme/v1.3/markers/b/mark_bs.png') no-repeat;background-size:20px,35px; color:#fff; line-height:20px; text-align:center;">${i +
                        1}</div>`
                });
                marker.push(marker1);
            }
